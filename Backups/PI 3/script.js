...
function testPassword() {
	...
	if(getCookie("messages") == "" || getCookie("messages") == null) {
...
//Log In Check - PhP
//Array maybe for news?
//Make Profile nice.
//Create front screen on Home. 
//Let Telegram do something :P
...
function createTelegrams(){
	var listgroupJS;
	var sidebarJS;
	if (document.getElementById('alreadyAThing') == null) {
		console.log("woo!");
		sidebarJS = document.createElement('div');
		sidebarJS.setAttribute('class','col-xs-6 col-sm-3');
		sidebarJS.setAttribute('id', 'alreadyAThing');
		listgroupJS = document.createElement('div');
		listgroupJS.setAttribute('class', 'list-group');
		listgroupJS.setAttribute('id', 'listgroupJSItem');
	} else {
		var elements = document.getElementsByClassName('list-group-item');
		while(elements.length > 0){
        	elements[0].parentNode.removeChild(elements[0]);
        }
        listgroupJS = document.getElementById('listgroupJSItem');
        sidebarJS = document.getElementById('alreadyAThing');
	}

	//for (element in news) {}
	var maxX = getCookie("messages");

	for (var x = 0; x<maxX; x++){
		var a = document.createElement('a');
		a.setAttribute('class', 'list-group-item');
		a.setAttribute('onclick', 'makeActive(this.id)');
		a.setAttribute('id', x);

		if (x == 0) {
			a.className += ' active';
		}
		a.innerHTML = "News: " + (maxX - x);
		listgroupJS.appendChild(a);
	}
	sidebarJS.appendChild(listgroupJS);
	if (document.getElementById("alreadyAThing") == null) { 
		document.getElementById("appendMe").appendChild(sidebarJS);
	}
}
